...
const fetchAward = async () => {
    ...
    loading.value = true;
    try {
        const response = await axios.get(`/api/awards/${route.params.id}`);
...
const saveAward = async () => {
    ...
    try {
        if (isNew.value) {
            const response = await axios.post('/api/awards', award.value);
            showToast('Auszeichnung erstellt', 'success');
            router.push({ name: 'awards.edit', params: { id: response.data.id } });
        } else {
            await axios.put(`/api/awards/${award.value.id}`, award.value);
            showToast('Auszeichnung gespeichert', 'success');
        }
    } catch (error) {
            ...
            <div class="col-span-2 space-y-12">
                ...
                <div class="bg-gray-50 p-4 pt-2 rounded-sm">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Auszeichnung *</label>
                            <textarea
                                v-model="award.title"
                                rows="4"
                                placeholder="z.B. Auszeichnung «best architects 24» Sanierung, Um- und Anbau Haus B4, Zürich"
                                class="w-full px-3 py-2 border border-gray-300 rounded-sm focus:ring-1 focus:ring-black focus:border-black"
                            ></textarea>
                        </div>
                    </div>
